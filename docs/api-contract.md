# API contract and baseline limits
...
`shingi`, `shingi-latest`, and the explicit baseline model ID select the local model. For client migration, `jev-latest` and `openjev` are accepted aliases too; the response always names Shingi. This does not route to or impersonate either upstream provider.
...
- Default context: 16,384 tokens. Oversized prompts fail rather than truncate. Larger configured contexts need memory and quality checks.
...
- [Bonsai GGUF, revision 6ed5e12](https://huggingface.co/prism-ml/Ternary-Bonsai-2-27B-gguf/tree/6ed5e12bf84b7a63069882c91dd9e9218647d17b), using native PQ2_0 weights and the existing Prism runtime at `d8f26eec76da6d09bb708bcba51ef64b8cd868a3`.
