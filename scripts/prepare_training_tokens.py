...
def main():
    p=argparse.ArgumentParser();p.add_argument('--model',type=Path,required=True);p.add_argument('--data',type=Path,default=Path('artifacts/decision-v2'));a=p.parse_args()
    ...
    tokenizer=NativeTokenizer('artifacts/bin/readout',a.model);excluded=[];count=0
    try:
    ...
    result={'data_manifest_sha256':sha(a.data/'manifest.json'),'tokens_sha256':sha(a.data/'tokenized.jsonl'),
            'executable_sha256':sha('artifacts/bin/readout'),'max_tokens':1024,'prepared':count,'excluded':excluded}
    with (a.data/'tokenized-manifest.json').open('x') as f:json.dump(result,f,indent=2)
    print(json.dumps({'prepared':count,'excluded':len(excluded),'data_manifest_sha256':result['data_manifest_sha256']}))
